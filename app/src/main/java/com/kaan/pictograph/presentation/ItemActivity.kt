...
import com.kaan.pictograph.data.model.ChartData
...
class ItemActivity : AppCompatActivity(), ISensor {
    ...
    companion object {
        ...
        private var lastAdded: Item ?= null
        var addedSensorIds: HashSet<String> = hashSetOf() // Store the added sensor ids

        fun addSensorItem(item: Item) {
            if (item.id.isNotBlank() && !addedSensorIds.contains(item.id)) {
                list += item
                addedSensorIds.add(item.id)
                lastAdded = item
            }
        }

        fun removeSensorItem(item: Item) {
            list -= item
            addedSensorIds.remove(item.id)
        }
    }
