package com.kaan.pictograph.data.repository
...
class CameraRepository @Inject constructor(private val api: CameraApi) {
    ...
    suspend fun uploadId(request: TextRecognitionRequest): List<TemperatureResponseAggregation> {
        Log.e(TAG, "uploadSensorId: start request")
        return try {
            val response = api.uploadId(request = request)
            ...
            Log.e(TAG, "uploadId: end request, parsed response=$resultList")
            resultList
        } catch (e: Exception) {
            Log.e(TAG, "uploadId: exception occurred: ${e.message}", e)
            emptyList()
        }
    }
